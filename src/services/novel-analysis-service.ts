...
export interface NovelAnalysisResult {
  ...
  characters: Array<{
    ...
    threeFaces?: { daily: string; pressure: string; hidden: string };
    evidence: string;
  }>;
...
- 着重提取人物三面性：每个重要角色需拆解为三个行为层面——日常面（安全环境下的默认表现，从性格主色调衍生）、压力面（冲突/威胁下的反应转变，从性格底色衍生）、隐藏面（特定条件下展现的隐藏特质，从性格点缀衍生）。三面性让角色立体、有层次，帮助 AI 在不同情境下表现不同的行为模式。
...
7. 人物条目建议结构（作为 content 字段）：
   ...
   - 记忆/闪回：容易被什么触发、会想起什么
   - 三面性：日常面（安全环境下的默认表现）、压力面（冲突/威胁下的反应转变）、隐藏面（特定条件下展现的隐藏特质）。用具体行为描写，不用抽象标签。
8. 关系条目建议结构：
   - 关系类型与情感动力
   - 双方日常互动模式
   - 玩家介入时可能引发的情绪变化或行为变化
  ...
  "characters": [
    {
      ...
      "threeFaces": { "daily": "日常面：安全环境下的默认表现", "pressure": "压力面：冲突/威胁下的反应转变", "hidden": "隐藏面：特定条件下展现的隐藏特质" },
      "evidence": "原文依据简述"
    }
  ],
  "relationshipMap": [
  ...
  "lorebookEntries": [
    {
      ...
      "keys": ["触发词"],
      "content": "世界书正文，字段化、可直接注入。RP-first：不要只写设定，要写 AI 如何表现。人物条目尽量覆盖：核心身份与动机、心理动态、行为模式、对话风格（含典型台词）、对他人态度、触发/消退条件、身体/环境细节、记忆/闪回、三面性（日常面/压力面/隐藏面，用具体行为描写）。关系条目覆盖：关系类型与动力、日常互动、玩家介入时的反应、张力点与底线。地点/势力/规则条目覆盖：触发场景、AI 应营造的氛围、对角色行为的约束或推动。物品条目覆盖：属性、功能、获取途径、对持有者的影响。包含：触发条件/关键事实/角色反应/可表现的对白或动作/写作注意。",
...
拆分要求：
- 主要人物至少尝试生成：核心设定、外貌、着装/视觉状态、行为逻辑、三面性（日常面/压力面/隐藏面）、关键关系。
