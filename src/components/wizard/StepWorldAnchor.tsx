...
export function StepWorldAnchor({
  ...
  return (
    <div>
      {/* Header */}
      <StepHeader title={t('worldAnchor.title')} subtitle={t('worldAnchor.intro')} />
      ...
      {anchorEntries.length > 0 && (
        <div className="mt-6">
          <div className="flex items-center gap-2 mb-3">
            <h3 className="text-sm font-semibold text-themed">
          ...
          <div className="space-y-2 sm:space-y-3">
            {anchorEntries.map(({ entry, index }) => (
              <div
                key={entry.id}
                className="relative rounded-lg border p-1 surface-a-30"
                style={{ borderColor: themeAlpha('warning', 25) }}
              >
                <div className="mb-1 flex items-center gap-1.5 text-[10px] px-2 pt-1 text-themed-secondary">
                  <span
                    className="rounded border px-1.5 py-0.5"
                    style={{ borderColor: themeAlpha('warning', 40), backgroundColor: themeAlpha('warning', 12), color: 'var(--color-status-warning)' }}
                  >
      ...
      {anchorEntries.length === 0 && (
        <div className="text-center py-10 border border-dashed rounded-xl text-themed-muted border-themed">
